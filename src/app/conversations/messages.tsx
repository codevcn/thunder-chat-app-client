...
import { EEventNames, EPeriodsOfStickyTime, ETimeFormats } from "@/utils/enums"
...
const CreatorMessage = ({
    ...
    const set_animate_new_msg = () => isNewMsg && animate ? '-translate-x-14 translate-y-4 opacity-0' : 'translate-x-0 translate-y-0 opacity-100'
    ...
    return (
        ...
        >
            <span></span>
            <div className={`${set_animate_new_msg()} transition duration-200 bg-regular-violet-cl rounded-t-2xl rounded-bl-2xl py-1.5 px-2 relative`}>
                <p className="text-sm inline break-all">
                    {content}
                </p>
                <div className="float-right ml-3 relative right-0 top-1">
                    <span className="text-xs text-regular-creator-msg-time-cl">
                        {time}
                    </span>
...
const RecipientMessage = ({
    ...
    return (
        <div className={`${''} bg-regular-bg-darkGray-cl rounded-t-2xl rounded-br-2xl pt-1.5 pb-2 px-2 w-fit relative`}>
            <p className="text-sm inline break-all">
                {content}
            </p>
...
export const Messages = memo(({
    ...
    const publishScrollToBottomMsgEvent = () => {
        messages_container_ref.current?.addEventListener(EEventNames.SCROLL_TO_BOTTOM_MSG, (e) => {
            if (ScrollToBottomEventor.isThisCustomEvent(e)) {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            messages_container_ref.current?.removeEventListener(EEventNames.SCROLL_TO_BOTTOM_MSG, () => { })
        }
    }, [])
    ...
    const map_message = (messages: TConvMessage[], user: TUserWithoutPassword) => messages.map((message, index) => {
        const sticky_time = index > 0 ? handle_sticky_time(messages[index - 1].createdAt, message.createdAt) : handle_sticky_time_of_first_msg(message.createdAt)
        ...
        <>
            {createPortal(
                <ScrollToBottomMessageBtn
                    messagesContainerRef={messages_container_ref}
                />,
                document.body
            )}
            ...
            >
                {
                    fetchedMsgs && user ? (
                        messages && messages.length > 0 ?
                            <Flex
                                className="py-2 box-border w-messages-list-width gap-y-2"
                                vertical
                            >
